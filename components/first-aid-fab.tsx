...
export function FirstAidFab({
  ...
  const isTagTrigger = triggerMode === "tag";

  const defaultAnchor: ViewStyle = isTagTrigger
    ? { top: 10, right: 10 }
    : { bottom: Math.max(insets.bottom, 10) + 66, right: 16 };
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      const historyForReply: ChatKbMessage[] = [...messages, userMsg].map(
        (m) => ({
          role: m.role,
          text: m.text,
        }),
  ...
  return (
    <View
      ...
      style={[styles.root, defaultAnchor, anchorStyle]}
    >
      {/* ── CHAT BOX ── */}
      {open && (
        <View
          style={[styles.chatPopover, isTagTrigger && styles.chatPopoverTag]}
        >
          <View
            style={[
              styles.box,
          ...
          >
            ...
            <View
              style={[
                styles.header,
                { backgroundColor: headerBg, borderBottomColor: borderClr },
              ]}
            >
            ...
            <FlatList<Msg>
              ...
              renderItem={({ item }) => (
                <View
                  style={item.role === "bot" ? styles.rowBot : styles.rowUser}
                >
                  {item.role === "bot" && (
                  ...
                  <View style={styles.rowBot}>
                    ...
                    >
                      <View style={styles.typingRow}>
                        <View
                          style={[styles.dot, { backgroundColor: muted }]}
                        />
                        <View
                          style={[
                            styles.dot,
            ...
            <View
              style={[
                styles.inputBar,
                { borderTopColor: borderClr, backgroundColor: headerBg },
              ]}
            >
              <TextInput
                style={[
      ...
      <Animated.View style={{ transform: [{ scale: fabScale }] }}>
        <Pressable
          onPress={toggleOpen}
          style={[styles.fab, isTagTrigger ? styles.tagTrigger : null]}
        >
          {isTagTrigger ? (
            <Text
              style={[
                styles.tagText,
                { color: isDark ? "#E5E7EB" : "#0F172A" } as TextStyle,
              ]}
            >
              {triggerLabel}
            </Text>
