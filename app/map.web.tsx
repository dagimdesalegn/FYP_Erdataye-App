...
export default function MapScreen() {
  ...
  const lat = location?.coords.latitude ?? 9.02;
  const lng = location?.coords.longitude ?? 38.75;
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ScrollView style={[styles.dataPanel, { backgroundColor: cardBg }]}>
        ...
        {/* Ambulances */}
        <ThemedText style={[styles.sectionTitle, { color: textColor }]}>🚑 Ambulances ({ambulances.length})</ThemedText>
        {ambulances.length === 0 && <ThemedText style={[styles.emptyText, { color: subText }]}>No ambulances available</ThemedText>}
        {ambulances.map((amb) => (
        ...
        {/* Emergencies */}
        <ThemedText style={[styles.sectionTitle, { color: textColor }]}>⚠️ Emergencies ({emergencies.length})</ThemedText>
        {emergencies.length === 0 && <ThemedText style={[styles.emptyText, { color: subText }]}>No active emergencies</ThemedText>}
        {emergencies
          .filter((e) => e.latitude !== 0 || e.longitude !== 0)
          .map((emergency) => (
            <View key={emergency.id} style={[styles.card, { borderColor: '#EF4444' }]}>
              <ThemedText style={[styles.cardTitle, { color: textColor }]}>
        ...
        {/* Hospitals */}
        <ThemedText style={[styles.sectionTitle, { color: textColor }]}>🏥 Hospitals ({hospitals.length})</ThemedText>
        {hospitals.length === 0 && <ThemedText style={[styles.emptyText, { color: subText }]}>No hospitals found</ThemedText>}
        {hospitals.map((h) => (
...
const styles = StyleSheet.create({
  ...
  mapContainer: { width: '100%', height: '50%' },
  controls: {
    position: 'absolute',
    right: 16,
    top: 16,
  ...
  sectionTitle: { fontWeight: 'bold', fontSize: 16, fontFamily: Fonts.sans, marginTop: 12, marginBottom: 8 },
