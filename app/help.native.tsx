...
export default function HelpScreen() {
  ...
  const mockAmbulances = React.useMemo(() => {
    if (!userCoord) return [];
    return [
      { id: 'AMB-102', name: 'Ambulance 102', etaMin: 4, latitude: userCoord.latitude + 0.0022, longitude: userCoord.longitude - 0.0016 },
      { id: 'AMB-211', name: 'Ambulance 211', etaMin: 7, latitude: userCoord.latitude - 0.0018, longitude: userCoord.longitude + 0.002 },
      { id: 'AMB-305', name: 'Ambulance 305', etaMin: 10, latitude: userCoord.latitude + 0.0031, longitude: userCoord.longitude + 0.0012 },
    ];
  }, [userCoord]);

  const mapHeight = Math.max(280, Math.min(420, Math.round(screenHeight * 0.48)));
        ...
        <ThemedView style={[styles.hero, { borderColor: isDark ? '#2E3236' : '#EEF2F6' }]}>
          <View style={styles.heroTopRow}>
            ...
            <View style={styles.heroTextCol}>
              ...
              >Nearby ambulances show on the map.</ThemedText>
            </View>
          </View>
          ...
          >
            {region ? (
              ...
              <View style={styles.mapPlaceholder}>
                <MaterialIcons name="my-location" size={22} color={isDark ? '#E6E9EC' : '#0F172A'} />
                <ThemedText style={[styles.mapPlaceholderText, { color: isDark ? '#A3AAB3' : '#64748B' }]}
                >Enable location to show nearby ambulances.</ThemedText>
              </View>
            )}
          </View>
        </ThemedView>
